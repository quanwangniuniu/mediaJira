...
function buildCacheKey(opts: UseCalendarViewOptions): string {
  const { viewType, currentDate, calendarIds, activeEventTypes } = opts;
  const baseDate = startOfDay(currentDate).toISOString();
  const ids = (calendarIds || []).slice().sort().join(",");
  const types = (activeEventTypes || []).slice().sort().join(",");
  return `${viewType}:${baseDate}:${ids}:${types}`;
}
    ...
    request
      .then(async (response) => {
        ...
        try {
          ...
          const allDerived = derivedResponse.data.map(derivedEventToEventDTO);
          const activeTypes = options.activeEventTypes;
            ...
            activeTypes && activeTypes.length > 0
              ? allDerived.filter((e) => {
                  try {
                    ...
                    return activeTypes.includes(meta.event_type);
                  } catch {
                    return true;
                  }
                })
          ...
          // Derived events failure does not affect the display of the main calendar
