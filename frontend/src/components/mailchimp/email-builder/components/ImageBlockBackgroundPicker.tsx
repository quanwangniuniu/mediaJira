...
import { BlockBoxStyles } from "../types";

interface ImageBlockBackgroundPickerProps {
  currentStyles: Partial<BlockBoxStyles>;
  handleStyleChange: (styleUpdates: Partial<BlockBoxStyles>) => void;
  setIsImageBlockBackgroundPickerOpen: (open: boolean) => void;
}

const ImageBlockBackgroundPicker: React.FC<ImageBlockBackgroundPickerProps> = ({
  currentStyles,
  handleStyleChange,
  ...
  const handleBgSelect = (color: string) => {
    ...
    setIsImageBlockBackgroundPickerOpen(false);
  };
  ...
  return (
    <div className="flex-1 flex flex-col bg-white min-h-0 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
        <button
          onClick={() => setIsImageBlockBackgroundPickerOpen(false)}
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            {emailColors.map((item, index) => {
              const isSelected =
                (item.isTransparent && !currentStyles.backgroundColor) ||
                currentStyles.backgroundColor === (item as any).color;
              return (
                <button
                  key={index}
                  onClick={() => {
...
export default ImageBlockBackgroundPicker;
