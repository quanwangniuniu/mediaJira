...
export default function CreateSpreadsheetModal({
  ...
  const [formData, setFormData] = useState<CreateSpreadsheetRequest>({
    name: '',
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = 'Spreadsheet name is required';
    } else if (formData.name.trim().length > 200) {
      newErrors.name = 'Spreadsheet name cannot exceed 200 characters';
    }
        ...
        <div className="relative overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-gray-100">
          ...
          <div className="px-8 pt-8 pb-4 border-b border-gray-100">
            <div className="space-y-1">
              <h2 className="text-2xl font-semibold text-gray-900">Create Spreadsheet</h2>
              <p className="text-gray-600">Give your spreadsheet a name to get started.</p>
            </div>
          </div>
