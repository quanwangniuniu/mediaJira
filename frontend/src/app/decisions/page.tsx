...
import type { AgentSpreadsheet } from '@/types/agent';
...
const DecisionsPage = () => {
  ...
  // Generate from spreadsheet modal state
  const [generateModalProjectId, setGenerateModalProjectId] = useState<number | null>(null);
  const [spreadsheets, setSpreadsheets] = useState<AgentSpreadsheet[]>([]);
  const [selectedSpreadsheetId, setSelectedSpreadsheetId] = useState<number | ''>('');
  const [loadingSpreadsheets, setLoadingSpreadsheets] = useState(false);
  const [generating, setGenerating] = useState(false);

  const handleOpenGenerateModal = async (project: ProjectData) => {
    setGenerateModalProjectId(project.id);
    setSelectedSpreadsheetId('');
    setLoadingSpreadsheets(true);
    try {
      const list = await AgentAPI.listSpreadsheets(project.id);
      setSpreadsheets(list);
    } catch {
      toast.error('Failed to load spreadsheets.');
      setGenerateModalProjectId(null);
    } finally {
  ...
  const handleGenerateDecision = async () => {
    if (!generateModalProjectId || !selectedSpreadsheetId) return;
    setGenerating(true);
    try {
      const result = await AgentAPI.generateDecisionFromSpreadsheet(
        Number(selectedSpreadsheetId),
        generateModalProjectId
      );
      toast.success('Decision generated successfully.');
      setGenerateModalProjectId(null);
      router.push(`/decisions/${result.decision_id}?project_id=${generateModalProjectId}`);
    } catch (error: any) {
      const message = error?.response?.data?.detail || 'Failed to generate decision.';
      toast.error(message);
    } finally {
      setGenerating(false);
    }
          ...
          >
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => handleOpenGenerateModal(project)}
                  className="inline-flex items-center gap-1.5 rounded-md bg-violet-600 px-3 py-1 text-xs font-semibold text-white hover:bg-violet-700"
                >
  ...
  return (
    <Layout>
      ...
      {/* Generate Decision from Spreadsheet Modal */}
      {generateModalProjectId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-base font-semibold text-gray-900">Generate Decision from Spreadsheet</h2>
              <button
                type="button"
                onClick={() => !generating && setGenerateModalProjectId(null)}
            ...
            <p className="mb-4 text-sm text-gray-500">
              Select a spreadsheet from this project. The AI will analyze its data and automatically generate a decision draft.
            </p>
            {loadingSpreadsheets ? (
              <div className="py-6 text-center text-sm text-gray-400">Loading spreadsheets...</div>
            ) : spreadsheets.length === 0 ? (
              <div className="py-6 text-center text-sm text-gray-400">No spreadsheets found in this project.</div>
            ) : (
              <select
                value={selectedSpreadsheetId}
                onChange={(e) => setSelectedSpreadsheetId(e.target.value === '' ? '' : Number(e.target.value))}
            ...
            <div className="mt-6 flex justify-end gap-2">
              <button
                type="button"
                onClick={() => setGenerateModalProjectId(null)}
                disabled={generating}
              ...
              <button
                ...
                disabled={!selectedSpreadsheetId || generating || loadingSpreadsheets}
