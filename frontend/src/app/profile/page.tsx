...
function ProfilePageContent() {
  ...
  const workedOnTasks = useMemo(() => {
    ...
    if (!tasks || tasks.length === 0) {
      console.log('[Profile] No tasks available, using test data');
      return testData;
    }
    ...
    // If no matching tasks found, show all tasks for testing, or fallback to test data
    if (userTasks.length === 0 && tasks.length > 0) {
      console.log('[Profile] No matching tasks found. Showing all tasks for testing');
      ...
      // If still no tasks after mapping, use test data
      if (mappedTasks.length === 0) {
        console.log('[Profile] No tasks after mapping, using test data');
        return testData;
      }
    ...
    // If no user tasks and no tasks at all, use test data
    if (userTasks.length === 0) {
      console.log('[Profile] No user tasks found, using test data');
      return testData;
    }
