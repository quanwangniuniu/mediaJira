...
=======
      // Jira decision: for explicit-save fields, only update committed value after success.
>>>>>>> Stashed changes
      setFieldStateValue(fieldKey, {
        optimisticValue: commitStrategy === 'explicit' ? newValue : newValue,
        isSaving: true,
        error: null,
      });
      ...
      try {
        ...
        const committedValue = (result ?? newValue) as TValues[TKey];
        setFieldStateValue(fieldKey, {
          committedValue,
          optimisticValue: undefined,
          isSaving: false,
          error: null,
          draftValue: commitStrategy === 'explicit' ? committedValue : currentState.draftValue,
        });
        return true;
      } catch (error) {
        if (requestIdRef.current[fieldKey] !== requestId) {
...
=======
          // Jira decision: keep draft for high-cost fields so users do not lose input.
>>>>>>> Stashed changes
          draftValue: config.preserveDraftOnError ? currentState.draftValue : undefined,
        });
        return false;
      }
    },
  ...
  const getFieldState = useCallback(
    <TKey extends keyof TValues>(fieldKey: TKey) => {
      ...
      return {
        ...state,
        value: (state.optimisticValue ?? state.committedValue) as TValues[TKey],
      };
    },
    [fieldState]
  );
