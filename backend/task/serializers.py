...
from task.models import Task, ApprovalRecord, TaskComment, TaskAttachment, TaskHierarchy, TaskRelation, AlertTask
...
class TaskLinkSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        try:
            # AlertTask was consolidated from alerting app into task app; use app_label
            # to avoid ContentType ambiguity if stale alerting.alerttask exists.
            if content_type == 'alerttask':
                ct = ContentType.objects.get(app_label='task', model='alerttask')
            else:
...
class AlertTaskSerializer(serializers.ModelSerializer):
    ...
    def validate_acknowledged_by(self, user: User | None) -> User | None:
        ...
        if not has_membership:
            raise serializers.ValidationError(
                "Acknowledging user must be a member of the task project."
            )
        return user
    ...
    def validate(self, attrs):
        ...
        if status_val in {"resolved", "closed"} and resolved_at is None:
