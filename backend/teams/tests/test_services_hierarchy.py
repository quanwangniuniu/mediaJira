...
class TestTeamHierarchy(TestCase):
    def test_get_child_teams(self):
        ...
        child1 = create_team(name="Child1", organization_id=1, parent_team_id=parent.id)
        child2 = create_team(name="Child2", organization_id=1, parent_team_id=parent.id)
        children = get_child_teams(parent.id)
        child_ids = [c.id for c in children]
        self.assertEqual(set(child_ids), {child1.id, child2.id})

    def test_get_parent_team(self):
        parent = create_team(name="Parent2", organization_id=1)
        child = create_team(name="Child3", organization_id=1, parent_team_id=parent.id)
    ...
    def test_is_valid_team_hierarchy_valid(self):
        parent = create_team(name="P3", organization_id=1)
        child = create_team(name="C3", organization_id=1, parent_team_id=parent.id)
        self.assertTrue(is_valid_team_hierarchy(parent.id, child.id))

    def test_is_valid_team_hierarchy_cycle(self):
        t1 = create_team(name="T1", organization_id=1)
        t2 = create_team(name="T2", organization_id=1, parent_team_id=t1.id)
