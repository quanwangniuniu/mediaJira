...
class WorkflowNodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = WorkflowNode
        fields = [
            'id', 'workflow_id', 'node_type', 'label', 'data',
    ...
    def validate(self, attrs):
        ...
        # Run model's clean method if updating existing instance
        if self.instance:
            node = self.instance
...
class WorkflowNodeCreateSerializer(WorkflowNodeSerializer):
    ...
    class Meta(WorkflowNodeSerializer.Meta):
        fields = ['id', 'node_type', 'label', 'data', 'created_at', 'updated_at', 'is_deleted']
        read_only_fields = ['id', 'created_at', 'updated_at', 'is_deleted']
...
class WorkflowConnectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'connection_type', 'condition_config', 'priority',
            'created_at', 'updated_at', 'is_deleted'
        ]
        read_only_fields = ['id', 'workflow_id', 'created_at', 'updated_at', 'is_deleted']
    ...
    def validate(self, attrs):
        ...
        # Validate connection rules using validator
        try:
            ConnectionValidator.validate_connection_create(source_node, target_node, workflow)
        except DjangoValidationError as e:
            raise serializers.ValidationError(e.message_dict)
        
        # Validate conditional connection requirements
        ...
        # Validate loop connection requirements
...
class BatchConnectionOperationSerializer(serializers.Serializer):
    ...
    # Use DictField instead of nested serializer - validation happens in view with context
